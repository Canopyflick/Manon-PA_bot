...
try:
    ...
    # Add missing columns
    add_missing_columns(cursor, 'manon_users', desired_columns_manon_users)
    conn.commit()
...
def main():
    ...
    try:
        ...
        if local_flag == True:
            ...
            token = os.getenv('LOCAL_TELEGRAM_API_KEY')
            token = os.getenv('LOCAL_TELEGRAM_API_KEY').strip()  # Strip any extra spaces or newlines

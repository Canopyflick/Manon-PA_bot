...
async def stats_command(update: Update, context):
    ...
    if message.entities:
        for entity in message.entities:
            if entity.type == "text_mention":  # Detect if it's a direct user mention
                ...
                logging.info(f"User ID: {mentioned_user_id}")
    ...
    try:
        ...
        logging.error(f"Error: {e} couldn't fetch user stats?'")
    ...
    if result:
        ...
        if today_goal_status == 'set':
            ...
            if (live_engagements_engaged) or \
                ...
                logging.info(f"escaped_emoji_string: {escaped_emoji_string}")
                ...
                if escaped_pending_emojis is not None:
                    if "🤝" in escaped_pending_emojis:
                        ...
                        logging.info(f"FINAL STRING: {escaped_combined_string}")
        ...
        try:
            ...
            logging.error("die gekke error weer (jaaa)")
    else:
    ...
    if await check_chat_owner(update, context):
        if len(context.args) > 0:  # Check if an argument (like a number) was provided
            ...
            if arg.isdigit():  # If the argument is a number, treat it as an amount
                ...
                special_type = arg
                logging.info(f"Special type gifted = {special_type}")
    ...
    if await check_chat_owner(update, context):
        if len(context.args) > 0:  # Check if an argument (like a number) was provided
            ...
            if arg.isdigit():  # If the argument is a number, treat it as an amount
                ...
                chat_id = update.effective_chat.id
                logging.info(f"Special type stolen = {special_type}")
...
async def handle_admin(update, context, type, amount=None):
    logging.info(f"entering handle_admin_command")
    try:
        user_id = update.message.reply_to_message.from_user.id
    except Exception as e:

...
async def _send_personalized_message(bot, chat_id, user_id, first_name, greeting, announcement, btc_change_message, specific_chat_id):
    """
    Composes and sends a personalized morning message to a single user. The message sequence includes:
    ...
    try:
        ...
        if goals_count == 0 and not btc_change_message and not specific_chat_id and random.random() > 0.15:
            return

...
| **_Nathan Calendar Bot** (`2XYWboSbyl78wzIC`) | Telegram Trigger → Switch (voice / photo / text) → AI Agent + Google Calendar tools → Telegram reply. Voice: OpenRouter `mistralai/voxtral-small-24b-2507-stt`. Photos & image documents: OpenRouter `qwen/qwen3-vl-8b-instruct` OCR pass (posters, email screenshots, invitations) → Calendar Agent. **Error workflow:** Nathan Error Notifier |

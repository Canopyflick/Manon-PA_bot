...
| **_Nathan Calendar Bot** (`2XYWboSbyl78wzIC`) | Telegram Trigger → Switch (voice / photo / text) → AI Agent + Google Calendar tools → Telegram reply. Voice: ElevenLabs `scribe_v2` (not on OpenRouter). Photos & image documents: OpenRouter `qwen/qwen3-vl-8b-instruct` OCR pass (posters, email screenshots, invitations) → Calendar Agent. **Error workflow:** Nathan Error Notifier |
